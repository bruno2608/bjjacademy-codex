...
> Use o guia mestre para regras completas de autenticação, papéis e fluxos de check-in (manual + QR). Este arquivo apenas registra o panorama rápido do MVP e as limitações atuais.

## Panorama rápido
- Login e papéis ainda dependem de mocks (`authMockService` + `useUserStore`), mas o `/login` já usa o design system novo (tema zdark aplicado via `data-theme`).
