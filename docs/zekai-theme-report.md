...
## 1. Resumo
- DaisyUI está configurado no `tailwind.config.js` com apenas dois temas personalizados (`Z-Dark` e `Z-Light`) e `darkTheme` apontando para `Z-Dark`, agora carregado diretamente sem fallback silencioso.
...
- `tailwind.config.js` carrega o plugin `daisyui` diretamente e registra apenas `themes: ['Z-Dark', 'Z-Light']` com `darkTheme: 'Z-Dark'`, mantendo o foco nos temas customizados.【F:tailwind.config.js†L1-L40】
...
- `styles/zekai-themes.css` registra oficialmente os temas via `@plugin "daisyui/theme"`, enquanto `styles/globals.css` importa esses temas e concentra os overrides componentizados baseados em tokens DaisyUI, sem paleta fixa.【F:styles/zekai-themes.css†L1-L60】【F:styles/globals.css†L1-L176】
