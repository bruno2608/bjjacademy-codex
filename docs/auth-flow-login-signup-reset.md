...
> Consulte o guia mestre para regras completas de login, cadastro, reset de senha, papéis e check-in. Este arquivo mantém apenas um retrato curto do estado técnico atual (mocks/stores) e diferenças conhecidas.

## O que existe hoje no código
- Login único em `/login` usando `authMockService` com senha piloto fixa e whitelist de usuários.
- Login único em `/login` usando `authMockService` com senha piloto fixa e whitelist de usuários; UI nova usa `ZkContainer`, card `.zk-card` e tokens DaisyUI (`bg-base-*`, `input-bordered`, `ZkAlert`).
