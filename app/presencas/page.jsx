  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex items-center gap-2 text-bjj-blue-200">
          <PieChart size={20} />
          <span>{turmaAtual ? turmaAtual.nome : 'Selecione uma turma'}</span>
        </div>
      </header>

      <div className="grid gap-6 lg:grid-cols-2">
        <section className="rounded-2xl bg-bjj-gray-800/70 p-5 shadow-lg ring-1 ring-bjj-gray-700">
          <div className="flex items-center justify-between gap-4 border-b border-bjj-gray-700 pb-3">
            <div>
              ...
              {aulaAtual && (
                <p className="text-sm text-bjj-gray-200">
                  Aula {aulaAtual.status === 'encerrada' ? 'encerrada' : 'prevista'} • {selectedDate}
                </p>
              )}
            </div>
          ...
          <div className="mt-4 space-y-3">
            ...
            {alunosDaChamada.map(({ aluno, matricula, presenca }) => {
              ...
              const statusInfo = presenca ? STATUS_LABELS[presenca.status] : STATUS_LABELS.PENDENTE;
              return (
                <div
                  key={matricula.id}
                  className="flex items-center justify-between rounded-xl border border-bjj-gray-700 bg-bjj-gray-900/60 px-4 py-3"
                >
                  <div>
                    ...
                    {faixa && <p className={`text-xs ${faixa.textColor}`}>{faixa.nome}</p>}
                  </div>
        ...
        <section className="rounded-2xl bg-bjj-gray-800/70 p-5 shadow-lg ring-1 ring-bjj-gray-700">
          <div className="flex items-center justify-between gap-4 border-b border-bjj-gray-700 pb-3">
            <div>
              ...
              <p className="text-sm text-bjj-gray-200">Filtros por período e turma da academia</p>
            </div>
            <div className="flex gap-2">
          ...
          <div className="mt-4 space-y-3">
            {pendencias.length === 0 && <p className="text-bjj-gray-100">Nenhuma pendência no período selecionado.</p>}
            {pendencias.map((item) => {
              const { presenca, turma, aula, aluno } = item;
              const statusInfo = STATUS_LABELS[presenca.status];
              return (
                <div
                  key={presenca.id}
                  className="flex items-center justify-between rounded-xl border border-bjj-gray-700 bg-bjj-gray-900/60 px-4 py-3"
