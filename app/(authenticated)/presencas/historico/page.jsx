...
export default function HistoricoPresencasPage() {
  ...
  const alunoPadrao = isStudent && user?.alunoId ? user.alunoId : firstAlunoId;
    ...
    <div className="space-y-4">
      <header className="flex flex-col gap-1">
        ...
        <p className="text-sm text-bjj-gray-300/80">
          Filtre por estudante e mês para acompanhar check-ins, pendências e aprovações.
        </p>
      </header>
      ...
      <div className="flex flex-wrap gap-3 rounded-2xl border border-bjj-gray-800 bg-bjj-gray-900/70 p-4 text-sm text-bjj-gray-200">
        {isStudent ? (
          <div className="flex items-center gap-2 text-xs uppercase tracking-[0.2em] text-bjj-gray-400">
            <Users size={16} /> {alunos.find((item) => item.id === alunoSelecionado)?.nome || 'Meu histórico'}
          </div>
        ) : (
          <>
            <div className="flex items-center gap-2 text-xs uppercase tracking-[0.2em] text-bjj-gray-400">
