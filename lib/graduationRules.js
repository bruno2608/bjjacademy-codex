...
export const orderBelts = (belts = []) => {
  const lista = Array.isArray(belts) ? belts : [];
  const conhecidos = new Set(lista.filter(Boolean));
  const ordenados = BELT_ORDER.filter((belt) => conhecidos.has(belt));
  const restantes = lista.filter((belt) => belt && !BELT_ORDER.includes(belt)).sort();
